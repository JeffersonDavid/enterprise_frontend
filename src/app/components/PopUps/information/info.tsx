...
<div style={{ display: show ? 'block' : 'none' }}>
    ...
    <div id="popup-container" className="fixed rounded-[10px] fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-2 bg-white border border-gray-300 shadow-md z-50">
        <button id="btn-cerrar" className="mbbtnheart absolute top-0 right-0 mt-[-10px] mr-[-10px] p-1 bg-red-700 rounded-full hover:bg-red-400 focus:outline-none" onClick={()=>setShow(false)}>
        ...
        <div className="bg-blue-100 border border-blue-500 text-blue-700 px-4 py-3 rounded-md" role="alert">
            ...
            <button className=" mbbtnheart mt-4 bg-red-700 text-white font-bold py-2 px-4 rounded hover:bg-red-400 focus:outline-none" onClick={()=>setShow(false)}>Aceptar</button>
        </div>
    </div>
    </div>
</div>
